...
class FlowDuoWindow(FLowDuo):
    ...
    def initialize_window(self, post_branch_length, pre_branch_length):
        ...
        branching_index, intersection_id = self.compute_intersection_id_in_weird_trees(branching_index, flow_1_ids,
    ...
    def compute_intersection_id_in_weird_trees(self, branching_index, flow_1_ids, flow_2_ids, intersection_id):
...
def get_flow_duos(n, metric=CURRENT_DUO_FLOW_METRIC):
    ...
    flow_duo_pairs = sorted(flow_duos.keys(), key=lambda x: flow_duos[x], reverse=True)
    result = []
...
def compute_flow_duos(min_length_flows, min_post_branching, min_pre_branching, metric,
    ...
    for conversation_id in conversation_ids:
        ...
        conversation_flow_duo_candidate, conversation_max = compute_highest_flow_delta(candidate_flows,
                                                                                       metric,
                                                                                       min_post_branching,
                                                                                       min_pre_branching, verbose)
        flow_duos[conversation_flow_duo_candidate] = conversation_max
    max_delta = max(flow_duos.values())
    logger.debug(
        "current_highest_delta is {}, after processing acceptable {} flowduos".format(max_delta, len(flow_duos)))
    return flow_duos


def compute_highest_flow_delta(candidate_flows, metric, min_post_branching, min_pre_branching, verbose):
